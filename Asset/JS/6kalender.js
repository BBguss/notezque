  ...
  async function ambilAcaraDariDatabase() {
    try {
      ...
      data.daftarAcara = acara.map((acara) => ({
        ...
        warna:
          data.warnaAcara[Math.floor(Math.random() * data.warnaAcara.length)],
      }));
  ...
  function tampilkanKalender() {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      // Hari bulan ini
        ...
        const tanggalFormat = `${tahun}-${String(bulan + 1).padStart(
          2,
          "0"
        )}-${String(hitungHari).padStart(2, "0")}`;
        kotakHari.dataset.tanggal = tanggalFormat;

        // Tandai hari ini
        if (
          hitungHari === new Date().getDate() &&
      ...
      // Hari bulan depan
        ...
        kotakHari.dataset.tanggal = `${tahun}-${
          bulan + 2
  ...
  function tampilkanDaftarAcara() {
    ...
    acaraTerurut.forEach((acara) => {
      ...
      elemenAcara.innerHTML = `
                <h4>${acara.judul}</h4>
                <p class="event-desc">${
                  acara.deskripsi || "Tidak ada deskripsi"
                }</p>
                <p class="event-date">${formatTanggalTampilan(
                  acara.tanggal
                )} pukul ${acara.waktu}</p>
